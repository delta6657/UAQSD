...
class ChatClient(Frame):
  ...
  def initUI(self):
    ...
    addClientLabel = Label(ipGroup, text="Añadir amigo: ")
    ...
    clientSetButton = Button(ipGroup, text="Añadir", width=10, command=self.handleAddClient)
    ...
    bottomLabel = Label(parentFrame, text="Chat p2p v.1.1.0")
  ...
  def handleSetServer(self):
    ...
    try:
        ...
        self.setStatus("Servidor escuchando en %s:%s" % serveraddr)
        ...
        self.setStatus("Error estableciendo servidor")
  ...
  def handleAddClient(self):
    if self.serverStatus == 0:
      self.setStatus("Primero establece la direccion del servidor")
      return
    clientaddr = (self.clientIPVar.get().replace(' ',''), int(self.clientPortVar.get().replace(' ','')))
    try:
        clientsoc = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        clientsoc.connect(clientaddr)
        self.setStatus("Conectado a cliente en %s:%s" % clientaddr)
        self.addClient(clientsoc, clientaddr)
        thread.start_new_thread(self.handleClientMessages, (clientsoc, clientaddr))
    except:
        self.setStatus("Erro conectandose al cliente")
  ...
  def handleClientMessages(self, clientsoc, clientaddr):
    ...
    clientsoc.close()
    self.setStatus("Cliente desconectado desde %s:%s" % clientaddr)
